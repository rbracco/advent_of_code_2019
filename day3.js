...
function createPositions(instructions){
    let pos = [{x: 0,y: 0,}]
    for (instruction of instructions){
        pos = addPositions(pos, instruction);
    }
    return pos;
}

function getMinDistance(posArray1, posArray2){
    let intersectDistances = []
    for (pos1 of posArray1){
        for(pos2 of posArray2){
            if (pos1.x === pos2.x && pos1.y === pos2.y && !(pos1.x === 0 && pos1.y ===0)){
                intersectDistances.push(Math.abs(pos1.x) + Math.abs(pos1.y));
            } 
        }
    }
...
function getMinSignalDelay(posArray1, posArray2){
    mn = 1000000000
    for (let i=1; i < posArray1.length; i++){
        for (let j=1; j < posArray2.length; j++){
            obj1 = posArray1[i]
            obj2 = posArray2[j]
            if (obj1.x === obj2.x && obj1.y === obj2.y && !(obj1.x === 0 && obj1.y ===0)){
                if (i+j < mn){
                    mn = i+j;
                    console.log(`new min found: ${mn}`);
                }
